    ...
    def do_it():
        if size == "0":
            for item in lb.curselection():
                ...
                quality = tk.Label(get_input, text="quality")
                quality_input = tk.Entry(get_input)
                def send():
                    compress(item, quality_input.get())
                quality.pack()
                quality_input.pack()
                submit = Button(get_input, text="Kaydet", command=send).place(x=30, y=100)
        elif size == "1":
            for item in lb.curselection():
                get_input = tk.Toplevel(new)
                get_input.geometry("300x300")
                ...
                def save():
                    scale(item, width_input.get(), height_input.get())
                width.pack()
                width_input.pack()
                height.pack()
                height_input.pack()
...
def compress(item, quality):
    subprocess.run("ffmpeg -i " + g[item] + " -c:v libx264 -crf " + quality + " " + quality + "New" + g[item])

def scale(item, width, height):
